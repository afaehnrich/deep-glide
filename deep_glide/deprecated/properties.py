import collections
#from jsbgym_new.abstractSimHandler import AbstractSimHandlerRL
import math
import numpy as np


class BoundedProperty(collections.namedtuple('BoundedProperty', ['min', 'max'])):
     bound = True
...
class Properties:
    ...
    #General properties
    infinity = BoundedProperty(-math.inf, math.inf)
    plusminusone = BoundedProperty(-1.,1.)
    #custom properties
    custom_dir_x = BoundedProperty(-1., 1)
    custom_dir_y = BoundedProperty(-1., 1)
    custom_dir_z = BoundedProperty(-1., 1)
    custom_alpha = BoundedProperty(-2.0 * math.pi, -2.0 * math.pi)
    custom_target_lat_geod_deg = BoundedProperty(-90, 90)
    custom_target_lng_geoc_deg = BoundedProperty(-180, 180)
    custom_pid_target_roll = BoundedProperty(-0.5 * math.pi, 0.5 * math.pi)
    #custom_pid_target_roll = BoundedProperty(-0.6, 0.6)
    custom_pid_target_pitch = BoundedProperty(-0.5 * math.pi, 0.5 * math.pi)
    custom_pid_target_heading = BoundedProperty(0, 2.0 * math.pi)
    custom_target_heading = BoundedProperty(0, 2.0 * math.pi)
    custom_noop = BoundedProperty(0.0, 1.0)
    custom_pos_dx_m = BoundedProperty(-20000., 20000.)
    custom_pos_dy_m = BoundedProperty(-20000., 20000.)
    custom_target_dx_m = BoundedProperty(-20000., 20000.)
    custom_target_dy_m = BoundedProperty(-20000., 20000.)
    custom_target_distance_m = BoundedProperty(-30000, 30000)
    custom_dubbin_heading_diff = BoundedProperty(-0.5, 0.5)
    custom_dubbin_target_x = BoundedProperty(-2500., 2500.)
    custom_dubbin_target_y = BoundedProperty(-2500., 2500.)
    custom_dubbin_turn_distance =BoundedProperty(-2000., 2000.)
    custom_dubbin_distance_arrived = BoundedProperty(100, 600)
    custom_dubbin_new_target = BoundedProperty(-1, 1)
    custom_target_distance_arrived = BoundedProperty(0, 2000)

    # position and attitude
    position_h_sl_ft = BoundedProperty(-1400., 85000.)
    attitude_pitch_rad = BoundedProperty(-0.5 * math.pi, 0.5 * math.pi)
    attitude_roll_rad = BoundedProperty(-math.pi, math.pi)
    attitude_psi_rad = BoundedProperty(0, 2.0 * math.pi)
    aero_beta_deg = BoundedProperty( -180, +180)
    position_lat_geod_deg = BoundedProperty(-90, 90)
    position_long_gc_deg = BoundedProperty(-180, 180)
    position_distance_from_start_mag_mt = Property()
    position_distance_from_start_lat_mt = BoundedProperty(0, 40000000)
    position_distance_from_start_lon_mt = BoundedProperty(0, 40000000)
    # velocities
    velocities_u_fps = BoundedProperty(-2200, 2200)
    velocities_v_fps = BoundedProperty(-2200, 2200)
    velocities_w_fps = BoundedProperty(-2200, 2200)
    velocities_v_north_fps = BoundedProperty(-2200, 2200)
    velocities_v_east_fps = BoundedProperty(-2200, 2200)
    velocities_v_down_fps = BoundedProperty(-2200, 2200)
    velocities_p_rad_sec = BoundedProperty(-2 * math.pi, 2 * math.pi)
    velocities_q_rad_sec = BoundedProperty(-2 * math.pi, 2 * math.pi)
    velocities_r_rad_sec = BoundedProperty(-2 * math.pi, 2 * math.pi)
    velocities_h_dot_fps = Property()
    # controls state
    fcs_left_aileron_pos_norm = BoundedProperty( -1, 1)
    fcs_right_aileron_pos_norm = BoundedProperty( -1, 1)
    fcs_elevator_pos_norm = BoundedProperty( -1, 1)
    fcs_rudder_pos_norm = BoundedProperty( -1, 1)
    fcs_throttle_pos_norm = BoundedProperty( 0, 1)
    gear_gear_pos_norm = BoundedProperty( 0, 1)
    # engines
    propulsion_engine_set_running =Property()
    propulsion_set_running =Property()
    propulsion_engine_thrust_lbs =Property()
    # controls command
    fcs_aileron_cmd_norm = BoundedProperty( -1., 1.)
    fcs_elevator_cmd_norm = BoundedProperty(-1., 1.)
    fcs_rudder_cmd_norm = BoundedProperty( -1., 1.)
    fcs_throttle_cmd_norm = BoundedProperty(0., 1.)
    fcs_mixture_cmd_norm = BoundedProperty(0., 1.)
    fcs_throttle_cmd_norm_1 = BoundedProperty( 0., 1.)
    fcs_mixture_cmd_norm_1 = BoundedProperty( 0., 1.)
    gear_gear_cmd_norm = BoundedProperty(0, 1)
    # simulation
    simulation_dt =Property()
    simulation_sim_time_sec =Property()
    # initial conditions
    ic_h_sl_ft = Property()
    ic_terrain_elevation_ft = Property()
    ic_long_gc_deg = Property()
    ic_lat_geod_deg = Property()
    ic_u_fps = Property()
    ic_v_fps = Property()
    ic_w_fps = Property()
    ic_p_rad_sec = Property()
    ic_q_rad_sec = Property()
    ic_r_rad_sec = Property()
    ic_roc_fpm = Property()
...
properties= {
    #custom properties
    'custom/alpha': BoundedProperty(-2.0 * math.pi, -2.0 * math.pi),
    'custom/target-lat-geod-deg': BoundedProperty(-90, 90),
    'custom/target-lng-geoc-deg': BoundedProperty(-180, 180),
    'custom/pid-target-roll': BoundedProperty(-0.5 * math.pi, 0.5 * math.pi),
    'custom/pid-target-pitch': BoundedProperty(-0.5 * math.pi, 0.5 * math.pi),
    'custom/pid-target-heading': BoundedProperty(0, 2.0 * math.pi),
    'custom/target-heading': BoundedProperty(0, 2.0 * math.pi),
    'custom/noop': BoundedProperty(0.0, 1.0),
    'custom/pos-dx-m': BoundedProperty(-20000., 20000.),
    'custom/pos-dy-m': BoundedProperty(-20000., 20000.),
    'custom/target-dx-m': BoundedProperty(-20000., 20000.),
    'custom/target-dy-m': BoundedProperty(-20000., 20000.),
    'custom/target-distance-m': BoundedProperty(-30000, 30000),
    'custom/dubbin-heading-diff': BoundedProperty(-0.5, 0.5),
    'custom/dubbin-target-x': BoundedProperty(-2500., 2500.),
    'custom/dubbin-target-y': BoundedProperty(-2500., 2500.),
    'custom/dubbin-turn-distance':BoundedProperty(-2000., 2000.),
    'custom/dubbin-distance-arrived': BoundedProperty(100, 600),
    'custom/dubbin-new-target': BoundedProperty(-1, 1),
    'custom/target-distance-arrived': BoundedProperty(0, 2000),

    # position and attitude
    'position/h-sl-ft': BoundedProperty(-1400., 85000.),
    'attitude/pitch-rad': BoundedProperty(-0.5 * math.pi, 0.5 * math.pi),
    'attitude/roll-rad': BoundedProperty(-math.pi, math.pi),
    'attitude/psi-rad': BoundedProperty(0, 2.0 * math.pi),
    'aero/beta-deg': BoundedProperty( -180, +180),
    'position/lat-geod-deg': BoundedProperty(-90, 90),
    'position/long-gc-deg': BoundedProperty(-180, 180),
    'position/distance-from-start-mag-mt': Property(),
    'position/distance-from-start-lat-mt': BoundedProperty(0, 40000000),
    'position/distance-from-start-lon-mt': BoundedProperty(0, 40000000),
    # velocities
    'velocities/u-fps': BoundedProperty(-2200, 2200),
    'velocities/v-fps': BoundedProperty(-2200, 2200),
    'velocities/w-fps': BoundedProperty(-2200, 2200),
    'velocities/v-north-fps': BoundedProperty(float('-inf'), float('+inf')),
    'velocities/v-east-fps': BoundedProperty(float('-inf'), float('+inf')),
    'velocities/v-down-fps': BoundedProperty(float('-inf'), float('+inf')),
    'velocities/p-rad_sec': BoundedProperty(-2 * math.pi, 2 * math.pi),
    'velocities/q-rad_sec': BoundedProperty(-2 * math.pi, 2 * math.pi),
    'velocities/r-rad_sec': BoundedProperty(-2 * math.pi, 2 * math.pi),
    'velocities/h-dot-fps': Property(),
    # controls state
    'fcs/left-aileron-pos-norm': BoundedProperty( -1, 1),
    'fcs/right-aileron-pos-norm': BoundedProperty( -1, 1),
    'fcs/elevator-pos-norm': BoundedProperty( -1, 1),
    'fcs/rudder-pos-norm': BoundedProperty( -1, 1),
    'fcs/throttle-pos-norm': BoundedProperty( 0, 1),
    'gear/gear-pos-norm': BoundedProperty( 0, 1),
    # engines
    'propulsion/engine/set-running':Property(),
    'propulsion/set-running':Property(),
    'propulsion/engine/thrust-lbs':Property(),
    # controls command
    'fcs/aileron-cmd-norm': BoundedProperty( -1., 1.),
    'fcs/elevator-cmd-norm': BoundedProperty(-1., 1.),
    'fcs/rudder-cmd-norm': BoundedProperty( -1., 1.),
    'fcs/throttle-cmd-norm': BoundedProperty(0., 1.),
    'fcs/mixture-cmd-norm': BoundedProperty(0., 1.),
    'fcs/throttle-cmd-norm[1]': BoundedProperty( 0., 1.),
    'fcs/mixture-cmd-norm[1]': BoundedProperty( 0., 1.),
    'gear/gear-cmd-norm': BoundedProperty(0, 1),
    # simulation
    'simulation/dt':Property(),
    'simulation/sim-time-sec':Property(),
    # initial conditions
    'ic/h-sl-ft': Property(),
    'ic/terrain-elevation-ft': Property(),
    'ic/long-gc-deg': Property(),
    'ic/lat-geod-deg': Property(),
    'ic/u-fps': Property(),
    'ic/v-fps': Property(),
    'ic/w-fps': Property(),
    'ic/p-rad_sec': Property(),
    'ic/q-rad_sec': Property(),
    'ic/r-rad_sec': Property(),
    'ic/roc-fpm': Property(),
    'ic/psi-true-deg': Property()
}
'''

  Property Catalog for c172p
    ...
    fcs/elevator-position-normalized (RW)
    fcs/roll-trim-sum (RW)
    fcs/left-aileron-control (RW)
    fcs/left-aileron-position-normalized (RW)
    fcs/right-aileron-control (RW)
    fcs/right-aileron-position-normalized (RW)
    fcs/yaw-trim-sum (RW)
    fcs/rudder-control (RW)
    fcs/rudder-position-normalized (RW)
    fcs/flaps-control (RW)
    fcs/flap-position-normalizer (RW)
    ...
    aero/function/velocity-induced-fps (R)
    aero/function/qbar-induced-psf (R)
    ...
    contact/unit[3]/x-position (RW)
    contact/unit[3]/y-position (RW)
    ...
    contact/unit[4]/x-position (RW)
    contact/unit[4]/y-position (RW)
    ...
    contact/unit[5]/x-position (RW)
    contact/unit[5]/y-position (RW)
    ...
    contact/unit[6]/solid (RW)
    contact/unit[6]/bumpiness (RW)
    contact/unit[6]/maximum-force-lbs (RW)
    contact/unit[6]/rolling_friction-factor (RW)
    contact/unit[6]/static-friction-factor (RW)
    contact/unit[6]/WOW (RW)
    contact/unit[6]/x-position (RW)
    contact/unit[6]/y-position (RW)
    contact/unit[6]/z-position (RW)
    contact/unit[6]/compression-ft (RW)
    contact/unit[6]/compression-velocity-fps (RW)
    contact/unit[6]/static_friction_coeff (RW)
    contact/unit[6]/dynamic_friction_coeff (RW)
'''