...
class RL_Live_DB:
    ...
    def _create(self, time, cfg, name = ""):
        dt_string = time.strftime("%d-%m-%Y %H.%M.%S")
        self.cur.execute(   'INSERT INTO experiments (time, cfg, episodes) '
                            'VALUES (?, ?, 1);', 
                            (dt_string, pickle.dumps(cfg)))
    ...
    def add_rewards(self, rewards):
        ...
        for agent, reward in enumerate(rewards):
            self.cur.execute( 'INSERT INTO rewards (experiment_id, episode, agent, reward) '
                              'VALUES (?, ?, ?, ?)',
                            (self.id, self.episodes, agent, reward) )
        self.episodes += 1
        self.db.commit()
